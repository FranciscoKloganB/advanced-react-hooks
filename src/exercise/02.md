...
It is possible to create custom hooks of increasing complexity...

In this exercise I learned how to create two types of custom hooks. One that
can be used to memoize the value returned by a function and another that tries
to reduce the amount of memory leaks that happen as a result of navigating away
from a page or unmounting a component while an async function is running.

To prevent all memory leaks from occurring when an async function is dispatched
and the component unmounts (for any reason) before the function returns, we would
need to implement an abort controller, but that can bring other kinds of complexity
to our app. So instead, we foccus on reducing the amount of times this error can occur
by not dispatching async functions if the component is unmounting or unmounted.

We learned the usefullness of the `useCallback` hook but we also realized that it
should be used only when we are deriving values (calculating something that depends on the
result of a side effect such as an `useEffect` hook) or performing complex computations
that really need memoization. Using the `useEffect` hook, alone, will in 99% of the cases
be the best choice and will also be easier to manage regarding dependency declaration,
minimizing and mitigating memory leaks of the asynchrounous functions. Multiple `useEffect`
can be declared in the same component to segregate responsabilities.

See:

- [Abortable fetch](https://developers.google.com/web/updates/2017/09/abortable-fetch)
- [Myths about useEffect](https://epicreact.dev/myths-about-useeffect/)
- [When to use useCallback,useMemo and, useEffect - stackoverflow](https://stackoverflow.com/questions/56910036/when-to-use-usecallback-usememo-and-useeffect)
- [When to use useCallback and useMemo - Kent C. Dodds](https://kentcdodds.com/blog/usememo-and-usecallback)
- [Five tips to help avoid React Hooks pitfals](https://kentcdodds.com/blog/react-hooks-pitfalls)
...
Requiring users to provide a memoized value is fine. You can document it as part of the API and expect people to just read the docs right? lol, that's hilarious.

😂 It'd be WAY better if we could redesign the API a bit so we (as the hook
developers) are the ones who have to memoize the function, and the users of our hook don't have to worry about it.
