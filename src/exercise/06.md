...
Sometimes we create custom hooks which bring valuable things to the developer
experience. However, when things go wrong, how do we debug things? What if
the custom hook we are belong to 3rd party lib that other people use?
How do we give them a way to figure out bugs? When we use `useDebugValue` hook
next to the hook we want to provide information about. It is a simple hook that receives
a log data, which is shown inside `React Developer Tools` in the browser and, if we wish
for it to run only (mitigate performance overhead caused by the hook) when the user is
using that tool during inspection. We can use the second argument `formatter` which will
ensure that the `useDebugValue` does not run unless console and the tool are visible! Note,
that this is only worth if it, if the computation in the debug value is expensive.

*Warning:* We can not use this hook inside components, only inside custom hooks.
