...
In this exercise I learned how to create a `Context` to store arbitrary values and
how to create `Providers` which enable child `Components` to access values within that
context. I also learned how to create a custom a `useContext` custom hook to faciliate
the usage of this feature and then implemented two different caches using Context
for the Pokemon app. One using the proposed `useReducer` and another using `useState`.
