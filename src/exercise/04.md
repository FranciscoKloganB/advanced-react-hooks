...
In this exercise I learned that, most of the time `useEffect` is the way to go
but sometimes, I might want to use `useLayoutEffect` instead. This can occur
in two situations: i) I want to ensure I run a side effect before any other
side effect on the component lifecycle runs; ii) if I need to run a side effect
after a Component is rendered by React but before it is painted by the browser.
E.g., when I want to change the position of the screen whenever I had a new item
to a list - which requires manipulating the DOM.
