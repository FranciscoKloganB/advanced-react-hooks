...
In this exercise I learned how to `useImperativeHandler`. This hook is
usefull, when we need to perform operations on the DOM from a parent
component through a child component. This hook, requires the use of
`React.forwardRef` as a sort-of high-order component around the Component
that is being wrapped. Forward ref allows the component to receive and utilize
`React.ref` as if it was declared within that component rather than its parent.

Quoting Kent C. Dodds: "We can use `useImperativeHandler` to expose imperative
methods to developers who pass a ref prop to our component which. We should only
do this we need to do something that needs to happen in  the DOM which would be
otherwise hard to implement declaratively.
